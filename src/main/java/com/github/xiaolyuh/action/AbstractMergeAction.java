...
public abstract class AbstractMergeAction extends AnAction {
    ...
    public void update(@NotNull AnActionEvent event) {
        ...
        // 已经初始化并且前缀是开发分支才显示
        event.getPresentation().setEnabled(isInit && StringUtils.startsWith(currentBranch, featurePrefix) && !isConflicts(event.getProject()));
    }
        ...
        if (flag == 0) {
            new Task.Backgroundable(project, getTaskTitle(project), false) {
                ...
                public void run(@NotNull ProgressIndicator indicator) {
                    ...
                    // 加锁
                    if (isLock() && !MrtfGitFlow.getInstance().lock(repository, getCurrentBranch(project))) {
                        GitCommandResult result = MrtfGitFlow.getInstance().getRemoteLastCommit(repository, getTargetBranch(project));
                        ...
                        NotifyUtil.notifyError(project, "Error",
                                String.format("发布分支已被锁定，最后一次操作信息：%s ;\r\n如需强行发布，请先点[发布失败]解除锁定，再点[开始发布]。", msg));
                        return;
                    }
                    ...
                    if (isMerge()) {
                        ...
                        GitCommandResult result = MrtfGitFlow.getInstance().mergeBranchAndPush(repository, currentBranch, targetBranch, getTagName(), errorListener);
                        if (result.success()) {
                    ...
                    if (isUnLock()) {
                        MrtfGitFlow.getInstance().unlock(repository);
                    }
    ...
    protected String getCurrentBranch(Project project) {
        return MrtfGitFlow.getInstance().getCurrentBranch(project);
    }
