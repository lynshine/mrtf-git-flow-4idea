...
public abstract class AbstractNewBranchAction extends AnAction {
    ...
    public void update(@NotNull AnActionEvent event) {
        ...
        event.getPresentation().setEnabledAndVisible(GitBranchUtil.isGitProject(project) && ConfigUtil.isInit(project));
    }
    ...
    public void actionPerformed(AnActionEvent event) {
        ...
        new Task.Backgroundable(project, getTitle(newBranchName), false) {
            ...
            public void run(@NotNull ProgressIndicator indicator) {
                ...
                if (isDeleteBranch()) {
                    // 删除分支
                    GitCommandResult result = MrtfGitFlow.getInstance().deleteBranch(repository, master, newBranchName, errorListener);
                    if (result.success()) {
                ...
                // 新建分支
                GitCommandResult result = MrtfGitFlow.getInstance().newNewBranchBaseRemoteMaster(repository, master, newBranchName, errorListener);
                if (result.success()) {
